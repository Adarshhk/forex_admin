        ...
        <div class="relative">
          ...
          <Dropdown v-model="isBrokerDropdownOpen">
            <button @click="() => {filters.broker_id = ''; isBrokerDropdownOpen = false}" :class="{'bg-white/10' : filters.broker_id == ''}"
              class="block text-left px-4 py-2 text-sm text-white hover:bg-[#ffffff11] w-full rounded">All</button>
            <button @click="() => {filters.broker_id = broker.id; isBrokerDropdownOpen = false}" v-for="broker in brokerFilterValues" :key="broker.id" :class="{'bg-white/10' : filters.broker_id == broker.id}"
...
const { manualPositions , brokerFilterValues } = storeToRefs(manualPositionStore);

// Filtered positions based on selected filters - without strategy filter for manual positions
const filteredPositions = computed(() => {
  return manualPositions.value.filter(pos => {
    ...
    // For manual manualPositions, we might want to filter out strategy manualPositions
    // This depends on your data structure - adjust as needed
    const isManualPosition = pos.is_manual === true; // Adjust based on your data structure

    return statusMatch && brokerMatch && isManualPosition;
  });
});
</script>
